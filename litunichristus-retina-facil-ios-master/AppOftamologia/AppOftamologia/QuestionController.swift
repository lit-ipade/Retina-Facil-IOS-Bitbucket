...
class QuestionController : UITableViewController{
    ...
    func addObservers() {
        NotificationCenter.default.addObserver(self, selector: #selector(self.viewDidAppear(_:)), name:
            NSNotification.Name.UIApplicationDidBecomeActive, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(self.appWillResign), name:
            NSNotification.Name.UIApplicationWillResignActive, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(self.appWillResign), name:
            NSNotification.Name.UIApplicationWillTerminate, object: nil)
    }
    ...
    override func viewForZooming(in scrollView: UIScrollView) -> UIView? {
        ...
        override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
            ...
            if indexPath.section == 0{
                if indexPath.row == 1 {
                    return 250
                    //return self.tableView.bounds.width*(question.image!.size.width/question.image!.size.height)/2
                }
            }
            ...
            if(indexPath.section == 1){
                ...
                let confirmButton = MGSwipeButton(title: "Confirmar", backgroundColor: appOrangeColor) {
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 1.1) {
                        ...
                        //answer.answerTime = currentQuestionTime
                        //answer.answeredKey = self.keys[cell.tag]
